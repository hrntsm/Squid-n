...
- 説明のある節には、その内容を実装している箇所を `**実装**` 行（`**実装**：` に続けて
  `squid_n_xxx::module::item` 形式のパスと一言の役割）で追記してください。参照は必ずコードを検索して
  実在を確認し、行番号は書きません（陳腐化するため）。既に実装への言及がある節には重複して書かず、
  入口となる関数は必要最小限に絞ります（目安 1〜2 個。独立した役割が複数ある場合はこの限りではありません）。
  ページ全体の実装を末尾の `## 実装` 節にまとめる方式のページでは、
  そちらを正とし、節ごとの追記は重複する場合に省略してよい
...
どの情報をどこへ書くか（用語・仕様・設計判断・経緯・検証記録の置き場）は、[dev_docs/README.md](dev_docs/README.md) の「ドキュメント配置ルール」を正とします。
