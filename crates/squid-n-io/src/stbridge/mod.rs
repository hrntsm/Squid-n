//! ST-Bridge（XML, 2.0.2）入出力。設計書 §12.5 / 仕様 `dev_docs/specs/P8_操作と連携.md` §7.1。
//!
...
//! - 基礎・杭・開口・パラペット。取り込み時にデータを欠落させる未対応要素は
//!   [`ImportReport`] の警告で必ず通知する（未知要素も部材/断面グループの
//!   直属子なら要素名で通知。fail-loud）。
//! - 通り芯（`StbAxes`）のうち円弧芯・放射芯・作図芯の**幾何**。通り名と所属節点は
//!   `AxisGroupKind::Other` のグループとして取り込むが、幾何を表す型がないため
//!   書き出しは平行芯（`StbParallelAxes`）のみ。
//!
//! # モジュール構成（1 ファイル 1 責務）
