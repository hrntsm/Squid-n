...
/// （立体モデルの [`crate::dynamic::eigen::solve_eigen`] と同じ規約。呼び出し側は
/// 返り値の長さを確認すること）。
///
/// 層に質量が 0 以下のものがあると `SolveError::InvalidInput` を返す。
...
/// 減衰算定用の1次固有円振動数 ω1。
///
/// 質量 0 以下の層は他層の正の質量の平均で置き換えてから固有値分解する。
/// 全層の質量が 0 以下、固有値分解自体が収束しない、または 1 次モードが特異のときのみ逆反復法へフォールバックする。
