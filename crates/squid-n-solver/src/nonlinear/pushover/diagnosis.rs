...
/// 接線剛性 `k_free`（自由 DOF 空間・CSC）の対角成分から、剛性がほぼ 0 の自由度と
...
fn diagonal_defects(
    dofmap: &DofMap,
    k_free: &faer::sparse::SparseColMat<usize, f64>,
...
pub(super) fn tangent_singular_diagnosis(
    ...
    let (zero, negative) = diagonal_defects(dofmap, k_free);
    if !zero.is_empty() {
        return format!(
...
pub(super) fn nonconvergence_detail(
    ...
    if !factorizable {
        return tangent_singular_diagnosis(model, dofmap, k_free, phase);
    }
    format!(
...
mod tests {
    ...
    fn test_diagnosis_names_zero_stiffness_dof() {
        ...
        let msg = tangent_singular_diagnosis(&model, &dofmap, &k, "初期接線剛性");
    ...
    fn test_diagnosis_reports_negative_diagonal() {
        ...
        let triplets: Vec<Triplet> = (0..n)
            ...
            .collect();
        let k = assemble_csc(n, triplets);
        let msg = tangent_singular_diagnosis(&model, &dofmap, &k, "接線剛性");
