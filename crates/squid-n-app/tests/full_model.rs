...
///
/// RC/SRC 梁の自重は柱フェース間の内法長で算定する（`squid_n_load::self_weight`）
/// ため、自重の同期は剛域の反映より後に行わなければならない。かつては
/// `generate_stories_action` が同期を先に行っており、1 回目の準備計算だけ
/// 剛域を反映しない過大な自重が DL に入り、2 回目の実行で初めて正しい値へ
/// 変わっていた。準備計算は各解析の実行前にも自動で走るため、
/// 「準備計算を実行した回数」で柱脚軸力・断面検定が変わる状態だった。
#[test]
