...
//! 準備計算が確定させる内容:
//!
//! 1. **剛域** — 部材端の剛域長 λ・柱フェース距離を接続部材のせいから自動算定し
//!    モデルへ反映する（`ZoneSource::Manual` の端は保護される）。
//! 2. **床荷重・自重・積載** — スラブの床荷重分配、躯体自重、用途別の積載荷重を
//!    標準荷重ケース（DL・LL(架構用)・LL(地震用)）へ同期する。
//! 3. **地震力（Ai 分布）** — 設計用固有周期 T から Rt・αi・Ai・Ci・Qi・Pi を
//!    算定し、水平力を EX・EY ケースへ同期する。
//! 4. **モデル整合性チェック** — 解析を妨げる不備（支点なし・断面未割当など）を
//!    検出する。
//!
//! 階（層）データ（階高・剛床・地震用重量）はこれらすべての前提であり、
//! 未定義の場合は [`App::run_preparation`] が自動生成する。
...
impl App {
    ...
    ///
    /// 階（層）が未定義の場合は先に自動生成する（階高・剛床・地震用重量は
    /// 剛域以外のすべての準備計算項目の前提のため）。既に階が定義されている
    /// 場合は手入力の重量・階種別を保護するため再生成しない（再生成は
    /// 解析設定パネルの「階の自動生成」で明示的に行う）。
    ///
