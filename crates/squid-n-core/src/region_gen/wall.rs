...
///
/// 再現するなら `test_isolated_dangling_column_does_not_create_spurious_boundary`
/// と同じ手筋（非軸並行の方位・実データ規模の大きな絶対座標）が有効なはず。この
/// フィルタも丸め誤差起因の面走査の乱れが対象であり、綺麗な数値の合成フィクスチャ
/// では誤差が閾値を超えず再現に失敗しやすい。
