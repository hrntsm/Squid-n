...
//!    例えば 0.5° の角度許容差は 50m 先で 436mm のずれになり、10mm の許容差としては粗すぎる。
...
//! 平面ごとに複数の境界へ自然に分解される。**この前提は緩めない**（複数面の節点をまとめて 1 つの境界にする近道を取らないこと）。
//!
//! # 面積・自重の計算について
//!
...
//! 面積・荷重は理想平面へ投影せず実座標から直接求める。芯ずれがあると、投影面積は自重計算で無視できない誤差になりうるため。
//!
//! # 計算量について
//!
...
//! # 未結線
//!
//! 本モジュールは幾何の検出のみを提供する。`WallRegion`/`WallPlate` への統合は未対応。
...
/// しかし `s`（`v・direction`）の計算に使う実座標の値が大きい（実測で 1〜2 万mm）と、
...
///
/// `MEMBER_AXIS_TOL_MM²`（100mm²）を採用したのは、既存の許容差定数を再利用しつつ、
/// 観測された丸め誤差（`1e-8` 程度）より 9 桁以上大きく、実在の壁面積
/// （小さくても数百万mm² オーダー）より 4 桁以上小さいためである。
...
mod tests {
    ...
    /// 孤立した柱（この構面上で他のどの部材とも接続しない、複数階に分割された柱）が
    /// 面走査を往復するだけの退化した閉路を作っても、境界を検出しない
    /// （`MIN_FACE_AREA_MM2` による回帰テスト）。
    ///
    /// 建物の柱は複数階にわたって節点が分割されるため、同一の柱に属する節点は
    /// XY が完全に一致する（Z のみ異なる）。この柱が構面上で孤立している
    /// （他のどの部材とも接続しない）場合、面走査は柱を往復する退化した閉路を
    /// たどる。射影後の座標 `(s, z)` は全節点で `s` が完全に一致するため理論上の
    /// 符号付き面積は厳密に 0 だが、`s`（`v・direction`）の計算に使う実座標の値が
    /// 大きく、かつ方向ベクトルが軸方向（見出し角 0°・90°）でない場合、シューレース
    /// 公式の丸め誤差により厳密な `> 0.0` 判定を通過しうる。
    ///
    /// 座標は実建物データ（`crates/squid-n-app/tests/fixtures/model.stb`）で
    /// 実際に発生したケースをそのまま再現する（もう一方の柱脚位置 (1000,1000) との
    /// 差分 (12800, 6400) が 2:1 の勾配となり、実測で観測された方向ベクトル
    /// `[0.8944271909999159, 0.4472135954999579]` と一致する）。軸方向の見出し角
    /// （0°・90°）では `s` の計算が単純な代入（掛け算の一方が 0 か 1）になり、
    /// 丸め誤差なしに厳密に 0.0 となってこのテストでは何も検出できない
    /// （`region_gen_finds_wall_bounded_regions` の面積正値チェックで検出した実バグ）。
    #[test]
    fn test_isolated_dangling_column_does_not_create_spurious_boundary() {
        let mut model = Model::default();
        // 直線の方向を定める、もう一方の柱脚位置（1 節点だけの孤立した柱。
        // これ単体は境界長 2 で `scan_faces` が捨てるため、退化閉路を作らない）。
        model.nodes.push(node(0, 1000.0, 1000.0, 0.0));
        model.nodes.push(node(1, 1000.0, 1000.0, 3000.0));
        model.elements.push(beam(0, 0, 1));

        // 孤立した柱（この構面上で他のどの部材とも接続しない）。4 節点・3 部材。
        for (k, z) in [200.0, 4700.0, 8700.0, 12700.0].into_iter().enumerate() {
            model.nodes.push(node(2 + k as u32, 13800.0, 7400.0, z));
        }
        for k in 0..3u32 {
            model.elements.push(beam(1 + k, 2 + k, 3 + k));
        }

        let boundaries = generate_wall_region_boundaries(&model);
        assert!(
            boundaries.is_empty(),
            "互いに孤立した柱どうしは境界を作らない: {boundaries:?}"
        );
    }
}
