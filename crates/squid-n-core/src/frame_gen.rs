...
//!
//! 生成した部材の断面は未割当のままとする。断面は利用者が決めるものであり、
...
use crate::model::{
    ...
    ElementKind, EndCondition, ForceRegime, LocalAxis, Model, Node, Story,
};
...
mod tests {
    ...
    fn test_generate_frame_counts_and_supports() {
        ...
        // 柱 = 格子点 6 × 3 層、大梁 = (X 方向 2 本 × 2 通り + Y 方向 1 本 × 3 通り) × 3 レベル。
