...
//!   されない独立した壁版として存在する。
//!
//! **未結線（2026-08-26 時点）**: 本型はまだ `region_gen::wall` の出力から組み立てる
//! 経路（`wall_region_rebuild`）・要素生成（D5）・ST-Bridge 入出力のいずれとも
//! 結線していない。詳細は `dev_docs/handoff/床領域・壁領域の再設計_申し送り.md` §5.8。
