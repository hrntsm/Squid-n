...
pub enum RegionAnchor {
    ...
    Point(NodeId),
    /// 床領域に取り付く（自立壁。床の上に立つ間仕切り等）。**荷重の出口（`region`）と
    /// 壁自体の平面上の長さ（`nodes`）を分けて持つ**のがこの形の特徴である。
    ///
    /// `nodes` は壁の始点・終点（節点参照。`Line` と同じ理由で節点移動に追随させる）。
    /// 面積（＝自重の計算）にはこの2点間の距離を使う。一方、荷重は `nodes` の両端では
    /// なく `region`（所属する床領域）へ渡し、等価な面荷重へならして分配する（D17）。
    /// **面荷重へならす時点で、壁が床領域内のどこにあるかという位置情報は失われる**
    /// （厳密に扱いたい場合は壁の直下に小梁を入れて主架構へ取り付ける運用とする）。
    /// 失われるのは位置であって長さではない。長さは自重に直接効くため、`nodes` を
    /// 持たず床領域 ID だけでは自重を算定できない。
    ///
...
pub struct FloorRegion {
    ...
    /// この床領域に属する床板の ID リスト。区画内が小梁で複数の打設単位に
