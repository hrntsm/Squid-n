//! 鉄筋コンクリート造柱梁接合部の**終局耐力**
//! （靭性保証型耐震設計指針の終局耐力）。
//!
//! [`crate::rc::joint`] が許容応力度検定を扱うのに対し、本モジュールは
//! 終局検定における接合部の終局せん断耐力 `Vju` と設計用接合部せん断力
//! `Qdu` の比（余裕率 `Vju/Qdu`）を算定する。節点まわりの応力集計は
//! 呼び出し側（[`crate::joint_wiring`]）の責務とする。

use crate::rc::joint::JointShape;

/// 接合部形状による係数 κ（靭性保証型耐震設計指針）。
///
