...
//! （靭性保証型耐震設計指針。部材耐力式として「靭性保証型設計指針」を
//! 選択した場合の経路。トラス機構＋アーチ機構の 3 式 min）。
...
///
/// (6.4.11) の平方根内は `L²+D²`。境界 `L/D=1.5` で (6.4.10) と連続する。
...
pub struct RcVbuInput {
    ...
    /// 1 段目主筋の周長合計 Σφ1 [mm]。
    pub sum_phi1: f64,
    /// 2 段目主筋の付着信頼強度 τbu2 [N/mm²]（ない場合 0）。
    pub tau_bu2: f64,
    /// 2 段目主筋の周長合計 Σφ2 [mm]（ない場合 0）。
    pub sum_phi2: f64,
    /// 横補強筋の間隔 s [mm]（λ に用いる）。
    pub s: f64,
    /// 中子筋の本数 Ns（λ に用いる）。
