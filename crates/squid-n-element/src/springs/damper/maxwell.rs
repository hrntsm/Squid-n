...
//!   `Ud` を求める。
//!
//! ## リリーフ特性（`relief_velocity`/`c2_ratio`）
//! `relief_velocity`（リリーフ速度 Vr）が指定されると、ダッシュポット力則の
//! 折れ線近似で表現する:
//!
//! - `|V| ≤ Vr`: `Fc = C0·sign(V)·|V|^α`。
//! - `|V| > Vr`: `Fc = sign(V)·(C0·Vr^α + C2·(|V|−Vr))`。
//!   `C2 = c2_ratio·C1`、`C1 = C0·α·Vr^(α−1)`（リリーフ点 Vr における
//!   リリーフ前の接線減衰係数）。`c2_ratio` が「リリーフ後の減衰係数比 C2/C1」
//!   を表す。
//!
//! `relief_velocity` が `None` の場合は全域で `Fc = C0·sign(V)·|V|^α`。
//!
//! `Δt<=0`（静的・線形解析）では不活性（力・剛性 0）。
...
pub struct MaxwellDamperElement {
    ...
    /// キャッシュ。`trial_elong == committed_elong` のとき
    /// `state_member_forces` はこの値を返す。
    committed_force: f64,
}
...
impl MaxwellDamperElement {
    ...
    ///
    /// `|V|` がリリーフ速度 Vr を超えると折れ線式
    /// （`Fc=sign(V)·(C0·Vr^α+C2·(|V|−Vr))`、`dFc/dV=C2`）に切り替わる。
    /// `|V|` は `1e-12` を下限にクランプする。
...
mod tests {
    ...
    fn test_maxwell_element_drives_axial_force() {
        ...
        let n = d.axial_force(1.0);
        assert!(n > 0.0);
        assert!((f.data[0] + n).abs() < 1e-9);
        assert!((f.data[6] - n).abs() < 1e-9);
    }
    ...
    /// リリーフ有効時、Vr 以下の速度域ではリリーフなしの場合と完全に一致すること。
    #[test]
    fn test_relief_matches_baseline_below_relief_velocity() {
        ...
        relief.relief_velocity = Some(1000.0);
