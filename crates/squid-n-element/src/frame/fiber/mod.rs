...
/// ファイバー梁の端部断面分割（幅方向・せい方向）。表示 API も同じ格子を使う。
pub(crate) const FIBER_NW: usize = 12;
pub(crate) const FIBER_ND: usize = 20;
...
impl FiberBeam {
    ...
    pub fn with_plastic_zone(
        ...
        Self::build_plastic_zone(data, model, lp, FIBER_NW, FIBER_ND, basis, kind)
    }
