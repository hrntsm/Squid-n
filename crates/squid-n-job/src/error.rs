...
//! （AGENTS.md「日本での利用を想定」。かつては app が日本語・MCP が英語の文字列を
//! 別々に組み立てており、同じ失敗でも入口によって文言が違った）。
//!
//! 種別を型で分けているのは、MCP が「収束不良なのか入力不備なのか内部エラーなのか」
//! を機械的に判別してレスポンスを変えられるようにするため（文字列では判別できない）。
...
impl JobError {
    ...
    ///
    /// 文言（`Display`）は利用者向けに変わり得るが、**このコードは安定**とする。
