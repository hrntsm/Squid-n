...
//! 柱（2 節点の鉛直梁）は上下節点へ 1/2 ずつ配分し、下階柱なしの RC/SRC 柱は
//! 梁最大せい相当の追加分を下端節点のみへ加算する。梁・ブレースの自重は等分布
//! 荷重の両端 1/2 ずつ、指定荷重ケースの部材荷重は単純支持梁の静定反力
//! （`static_reactions`）で両端に配分する（令88条の地震用重量算定における
//! CMoQo による梁せん断力 Q0 の実務的取扱いに相当。対称荷重では結果的に
//! 自重と同じ 1/2-1/2 になる）。
//!
