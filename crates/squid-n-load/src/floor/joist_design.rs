//! 床領域の分配結果（[`super::distribute_region`]）から二次部材小梁の設計部材力を求める。
//!
//! ST-Bridge 取り込み小梁（`Model::secondary_members`）の断面検定は、幾何から負担幅を
//! 再導出せず、荷重分配と同じ `BeamLoad`（`LoadTarget::Span`）を単純梁として重ね合わせる。
