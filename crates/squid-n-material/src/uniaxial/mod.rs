...
pub trait UniaxialMaterial: Send + Sync + Debug {
    ...
    /// 状態は内部に試行値として保持。
    ///
    /// **未経験（初期）状態で `strain = 0.0` を与えたときは、応力 0・接線剛性
    /// = その材料の初期弾性係数を返さなければならない**（実装の共通要件）。
    /// ファイバー要素はこの戻り値を断面の初期接線としてキャッシュし
    /// （`squid_n_element` の `GaussPoint::new`）、塑性化域考慮モデルはさらに
    /// そこから「弾性状態でヒンジ回転 0」を成立させる基準剛性を採る。ここで 0 を
    /// 返すと、弾性域にもかかわらず要素接線剛性が負になり増分解析が解けなくなる。
    /// 骨格式が原点で応力 0 になること（＝終局域のゼロクランプ条件と区別がつかないこと）
    /// に注意すること。
