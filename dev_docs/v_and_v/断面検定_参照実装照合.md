...
| B18 | RC 柱メカニズム ΣMy（梁/柱ヒンジ・両端梁降伏禁止） | ✅ 2026-08-12。方向ごとに My 欠落時はその方向を破棄して `2·Mu` 代替。**RC 矩形梁以外（SRC・円形等）が加力方向にあればその方向は常に 2·Mu**。部分スリット強制ヒンジは未配線 | `rc/column_mechanism.rs` |
| C4 | マニュアル差分の残置（2026-08-12 dig） | ❌ 壁開口低減の係数 1.1・壁側柱 j=0.8D・計算ルート連動 QD/n/pw・部分スリット強制柱ヒンジは採らない／未配線 | — |
