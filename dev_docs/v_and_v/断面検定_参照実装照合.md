...
| C4 | マニュアル差分の残置（2026-08-12 dig） | ❌ 壁開口低減の係数 1.1・壁側柱 j=0.8D・計算ルート連動 QD/n/pw・部分スリット強制柱ヒンジは採らない／未配線。**MCP は地震時短期で「長期⊕地震」の組合せ内力＋QD/メカニズム配線済み**（一本部材グループは未対応）。th_detail は**非線形 TH かつ長期重畳済み**のときのみ QD/メカニズムを配線（線形 TH では無効） | — |
