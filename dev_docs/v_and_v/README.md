...
> 解析コア（静解析・固有値解析・増分解析）の敵対的レビュー（2026-07 第2回）の結果は
> `dev_docs/v_and_v/解析コア_敵対的レビュー_2026-07.md` を参照。MPC/剛リンクのマスター拘束時の
> 無言無視・Rayleigh 減衰の重根発散・固定 λ 刻みの増分拡大・4節点壁への線材評価誤適用・
> 不静定次数の直交部材混入・弧長法フェーズのヒンジ追跡欠落などを回帰テスト付きで修正。
> 横断的な敵対的レビュー（2026-07）の結果は `dev_docs/v_and_v/adversarial_review_2026-07.md` を参照。
...
> その他の残る指摘は同レポートに記録。
> 変位制御フェーズはその後、載荷パターンを Ai 分布から頂部 1 点へすり替えていた欠陥
> （弾性でも性能曲線が V 字を描く）を比例荷重パターン保持の変位制御（Batoz–Dhatt 法）へ
> 再定式化して解消。`dev_docs/v_and_v/変位制御_荷重パターン保持_2026-07.md` を参照。
> P7（二次設計）の監査結果は `dev_docs/v_and_v/p7_review.md` を参照。
...
> P8（操作・連携）の監査結果は `dev_docs/v_and_v/p8_review.md` を参照。
...
> P4（材料・断面）の監査結果は `dev_docs/v_and_v/p4_review.md` を参照。
> #19: 包絡線（軟化・接線符号・連続性）・ひび割れ判定・MP 反転検知/ξ 更新を修正し、単軸履歴則 insta スナップショット追加で ✅。
> #19 追補（2026-07-29）: MenegottoPinto の反転則が漸近線交点を反転点に対して鏡映する
> 非標準実装で繰返し載荷時に応力が発散する欠陥（fy=345 に対し σ が -1005 N/mm² まで成長）を
> 発見し、Filippou et al. (1983) の交点更新則へ修正（±335〜350 N/mm² に収束）。あわせて
> Mander (1988) 拘束コンクリート包絡線（`mander.rs`）・Yassin (1994) の繰返し履歴
> `ConcreteCyclic`（`concrete_cyclic.rs`）を新規実装し、ファイバー要素・壁エレメントの
> 鋼材・主筋既定を Bilinear → MenegottoPinto へ変更した。詳細は
> `dev_docs/v_and_v/ファイバー材料モデル_論文照合_2026-07.md` を参照。
...
> 詳細・未実装項目は `dev_docs/v_and_v/非線形モデル_参照実装照合.md`。
> #26: 参照実装マニュアル「06 終局検定」照合で、これまで荒川mean式（`rc_qsu_simple`）しかなかった
...
> `dev_docs/v_and_v/終局検定_参照実装照合.md`。
> #15 追補（2026-07-27）: 増分解析の「ヒンジ発生と剛性低下」の実挙動検証で、
> (a) 材端集中ばね梁がばね変形に節点回転を用いており固定柱脚でヒンジが形成されず
> 剛性低下が一切生じない・降伏近傍で発散する定式化欠陥を発見し是正
> （`材端集中ばね梁_定式化レビュー_2026-07.md` §1.3）、(b) 変位法ファイバー要素は
> 端部ガウス点の積分重み（2Lp/L'）の制約で降伏後も要素剛性がほぼ低下しない
> （柱の Auto 判定がファイバーのため柱ヒンジの層剛性低下を表現できない）ことを記録。
> → その後、塑性化域考慮モデルを「全可撓長の弾性梁＋端部の塑性増分ヒンジ」の
> 直列モデルへ再定式化して解消（同日。弾性状態は弾性 Timoshenko 梁に厳密一致、
> 単柱検証で k0=189.8→kt=33.8 N/mm・Qu=2712N が全塑性崩壊荷重 Vp=2750N の 98.6%）。
> あわせて終了目標（目標変位に加え目標最大層間変形角、既定 1/150）を導入。
> 詳細は `増分解析_ヒンジ形成と剛性低下_検証_2026-07.md`。
> #15 追補（2026-07-30）: 利用者報告「長期の静解析は通るのに増分解析だけ落ちる」の原因が、
> NewRC コンクリート包絡線の**ひずみちょうど 0 での接線が 0**（終局域の応力ゼロクランプが
> 原点も巻き込んでいた）であることを特定し是正。ファイバー断面の初期接線キャッシュ
> （`GaussPoint::new` の `trial(0.0)`）→ 塑性増分ヒンジの弾性基準 `sec_ei` を経由して、
> 弾性域なのに要素接線剛性の対角が負になっていた。あわせて (a) 特異診断が拘束縮約**前**の
> 行列を走査しており剛床代表節点を必ず誤検出していた点、(b) 応力図が理論上ゼロの成分
> （ねじり解放時の Mx）の丸め誤差をフルスケール描画していた点を是正。
> 詳細は `増分解析_長期載荷の接線剛性破綻_2026-07.md`。
