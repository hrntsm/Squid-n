...
| [壁断面統一と正負別せん断耐力_2026-09.md](壁断面統一と正負別せん断耐力_2026-09.md) | 材料別実幅・SRC/CFT・任意角度・正負別壁ばね | 🔶（数値検証済み・実験照合未） |
| [耐震壁_敵対的レビュー_2026-07.md](耐震壁_敵対的レビュー_2026-07.md)（2026-09-11 面外ピン・非線形の壁柱断面力・内法質量・節点順・上下辺対応の修正と旧κ式削除・解析積分、軸方向部材荷重の内力復元を修正、側柱の形状未定義を入力エラー化、任意角度の端部解放を修正、非対称断面積分・矩形重複控除を整備、材料別の全形状を壁へ接続済み・正負別RC耐力を実装、軸力無効化時の形状・非軸剛性保持を検証、複合材料評価は承認済み・材料定数付き区間積分を追加） | 壁エレメント | 🔶 |
...
全解析種別で「CPU・単一スレッドでビット一致」を検証（R28）。並列／Parquet書込は値一致で検証（ビット一致保証外）。
