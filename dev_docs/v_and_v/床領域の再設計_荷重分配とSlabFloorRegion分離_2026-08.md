...
## 訂正（2026-09-16）: `resolve_edges_to_span` は割当領域の境界を正本とする

本文の「`resolve_edges_to_span` が `LoadTarget::Edge → Span` へ変換する」記述は当時の実装である。
後続の ADR 0020 統合で、囲まれた床板の辺は床板割当領域の境界（支持部材 ID と材軸区間）から
解決する形へ変えた。現在は辺の支持部材が主架構ならその要素の材軸区間へ、二次部材なら
`LoadTarget::Secondary` へ解決し、床板自身の境界節点対は使わない。現行仕様は
[床荷重の分配](../../docs/calc_basis/01_荷重/03_床荷重の分配.md) を参照。
