...
`dev_docs/v_and_v/原典照合リスト.md`「参照実装マニュアル計算編 05 非線形モデル」節を参照。
...
- T形/腰壁垂壁付き梁の αy 補正（矩形断面 αy の I 比補正）
- 腰（垂）壁付き大梁の曲げ・せん断終局、SRC 造耐震壁の終局せん断 3 式、
  SRC 梁の累加等価トリリニア、間柱ダンパーの 3 分割要素、
  ファイバー分割規則（対数分割・1.1 倍第1折点・剛性低下率 1/1000）、
  静的解析時の鋼材ファイバー逆行型

### 2026-07-13 追補2（既知ギャップの実装・是正）

- **材端バネ αy の菅野式配線を実装**: 材端曲げバネ（トリリニア骨格）の降伏点
  変形に用いる剛性低下率 αy を、RC 矩形断面の梁（水平材）では菅野式
  （`rc_capacity::rc_alpha_y_sugano`、pt=at/(b·D)、a=可撓長さ/2、n=Es/Ec）で
  算定する（`factory::flexural_alpha_y`）。柱（鉛直材、菅野式は軸力項を要する）・
  鉄骨・SRC・CFT・情報不足時は従来既定 0.3 のまま。
- **材端バネの Mc の √Fc 漏れを是正**: `factory::crack_moment` に残っていた
  `Mc = 0.56·Fc·Ze` を `0.56·√Fc·Ze` へ（本追補冒頭の是正と同一の誤りの残存
  箇所。beam_nonlinear・プッシュオーバー側は是正済みだった）。
