...
- `new()` がこれらを固定値で埋め、`with_params` で上書き可能とはいえ、デフォルト経路がハードコード。出典コメントもない。
...
- 再載荷「反対側の最大経験点を指向」、内側ループ「武田の規則」が未実装。`squid-n-element::shear_spring.rs:8` が `MemberSkeleton`（`hysteresis: HysteresisRule`）を保持するが、trial/commit/revert がないと P5 の集中ばね要素で M-θ 履歴を計算できない。
...
- 仕様 §7 フロー4「せん断・付着・抜出しによる回転を加算」。実装は `θ = φ·l/3` のみ（曲率分布を三角形と仮定）。せん断変形・鉄筋抜出し・付着すべりによる回転加算がない。
...
### 3.1 V&V 索引（`dev_docs/v_and_v/README.md`）に P4 項目が存在しない ★過小申告
`dev_docs/v_and_v/README.md:44-64` の索引 #1〜#18 に、P4 の核心（履歴則・ファイバ断面・スケルトン）の行がない。#15 プッシュオーバー（P5）、#16 壁（P5.5）、#17 時刻歴（P6）、#18 限界耐力（P12）はあるのに、**P4 の「履歴則」「ファイバ断面」「スケルトン自動算定」が欠落**。
...
- §6 `Fiber.y/z/area`、`SectionStrain.eps0/ky/kz`、`SectionForce.n/my/mz` に単位がない。
- §7 `MemberData.span [mm]` だけ明記。ファイバは mm か m か。曲率 ky/kz は 1/mm か 1/m か。モーメント my/mz は N·mm か kN·m か。
- 構造解析ライブラリでは解析系（P1/P2）と材料/断面系（P4）で単位を固定するか明示変換するのが鉄則。P3 で `MemberForcesAt` の単位不備が 1000 倍バグを生んだ（P3 レビュー §1.1）。P4 も同じ穴を抱え、実装 §1.9 で指摘したとおり `FiberSection` に単位 doc がない。
...
- §4「曲率: R = R0 − a1·ξ / (a2 + ξ)（ξ=反転後の塑性ひずみ振幅）」。ξ の更新式がない。
...
- §7 フロー2「折れ点の決め方は設計規準準拠」。DoD §8.3「手計算（規準式）と一致」。しかし §7/§8.3 に Mc, My, Mu の具体式がない。
...
7. **V&V 索引と pending_items.md の補完**
   - `dev_docs/v_and_v/README.md` の索引に「履歴則」「ファイバ断面」「スケルトン自動算定」を追加。現状は ❌。
   - `dev_docs/v_and_v/pending_items.md` に P4 セクションを新設し、§1/§2 の未達を記載。
8. **仕様書の改訂**
