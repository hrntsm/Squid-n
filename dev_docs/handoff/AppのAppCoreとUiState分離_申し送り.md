...
- **ヒンジ詳細ウィンドウの N-M 曲線キャッシュ（`hinge_mn_cache`）の鍵。** `MnCache` と同じ性質で、鍵が `(部材, 増分解析のステップ数)` だけのため断面の編集に追随しません。ただし `MnCache` ほど危険ではないと判断して本ブランチでは触っていません。断面を編集すると `Staleness::mark_edited` が `results_stale` を立て、ヒンジ図は増分解析の結果に基づく表示なので、利用者には「結果が古い」と警告が出ます。一方 MN 相関曲面ビューは結果ではなく断面から直に算定する表示なので、断面を編集しても警告が出ず、誤った曲面が正しい顔で出ていました。この差が優先順位の差です
