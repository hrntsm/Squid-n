...
- ~~**ヒンジ詳細ウィンドウの N-M 曲線キャッシュ（`hinge_mn_cache`）の鍵。**~~ **（2026-09-17 解消、Issue #286）** `MnCache` と同じ性質で、鍵が `(部材, 増分解析のステップ数)` だけのため断面の編集に追随しませんでした。当時は `MnCache` ほど危険ではないと判断して本ブランチでは触っていません。その後 `hinge_view_cache` へ再設計され、キーが要素 ID・選択 step・結果世代（`last_run`＋`results_stale`）・断面／材料／履歴則の指紋を含むようになり、断面・材料・履歴則の編集や同一ステップ数の再解析に追随します
