...
この申し送りでは、124 フィールドの `App` 構造体を `AppCore` と `UiState` へ分けた経緯と、その過程で見つかった危険側の誤表示を記録します。
...
実際、着手時点で 12 フィールド分の書き漏らしが残っていました。
そのうち `mn_view` の 1 件は、利用者が実際より大きい耐力を読み取りうる危険側の誤表示につながっていました。
...
なお `analysis_cfg` と `wave_library_selection` は、どちらも `.scz` の `SavedAnalysisSettings` に入るのに置き場所が違います。
前者は `AppCore` 直下（持ち越す）、後者は `ModelScoped`（捨てる）です。
減衰定数や刻み数は利用者の作業上の好みとして次のモデルでも意味を持つのに対し、波形は「この建物のためにこのファイルを選んだ」という結び付きしかないためです。
この使い分けは分割前からのもので、本ブランチでは変えていません。
...
旧 `load_model` は 53 フィールドを列挙してリセットしていました。
`ModelScoped` と `UiModelScoped` に入ったのは 66 フィールドなので、13 フィールドが列挙から漏れていたことになります。
...
| `ds_beta_u_by_story` / `ds_beta_u_unavailable` / `ds_rank_fallback_stories` | 保有水平耐力の警告表示 | 旧モデルの βu・ランクフォールバック警告が残る |
...
| `node_edit` | 節点座標の編集バッファ | 実害なし。`load_model` の末尾で `sync_node_edit` が全件を書き直すため |

実害の大きさには差があります。
`mn_view` だけが黙って誤った値を見せるもので、残りは古い状態が画面に残るか、`.scz` に不要な記録が入るかのどちらかです。

`ds_beta_u_by_story` 群は表示側が `displayed_pushover()` の内側にあり、`results` が捨てられる以上は画面に出ないので、実害はありませんでした。
...
- **`UiState` を丸ごと捨てる形にはしない。** gui 系 76 のうち `load_model` が捨てるのは 32 だけで、残る 44（ドック配置・カメラ・配色・表示トグル）は利用者の作業環境として持ち越す。`UiState` ごと `Default::default()` にすると、モデルを開くたびに画面が初期化されて作業にならない
- **`UiState` をフラットにして、リセット関数へ対象を列挙する案も採らない。** それは `App` にあった書き漏らしの構造を `UiState` の中へ引っ越すだけで、目的を達成しない。だから `scoped` と `view` に型で分け、代入 1 つで捨てられる形にした
...
- ~~**ヒンジ詳細ウィンドウの N-M 曲線キャッシュ（`hinge_mn_cache`）の鍵。**~~ **（2026-09-17 解消、Issue #286）** `MnCache` と同じ性質で、鍵が `(部材, 増分解析のステップ数)` だけのため断面の編集に追随しませんでした。当時は `MnCache` ほど危険ではないと判断して本ブランチでは触っていません。その後 `hinge_view_cache` へ再設計され、キーが要素 ID・選択 step・結果世代（`last_run`＋`results_stale`）・断面／材料／履歴則の指紋に加え、解決レジーム（材端集中ばねか）と壁側柱判定を含むようになり、断面・材料・履歴則の編集、同一ステップ数の再解析、stale 中の剛床・壁の追加削除に追随します
