...
- **変更後**: 描き方を `viewer::element_draw_shape`（`DrawShape::Line` / `Polygon` /
  ...
  （インラインの `matches!(Wall | Shell)`）」「ピック対象にするか（`pick_nearest_member` の
  インラインの `matches!(PanelZone)`）」という同じ分類が別々に存在していたため、
  これらも `element_draw_shape` から導く形に統一した（`draws_as_line` は
  `== DrawShape::Line` の薄いラッパ、ピック除外は `== DrawShape::None`）。
  ピックの条件は書き換えの前後で全要素種別について等価であり、面要素は従来どおり
  ピックできる。
...
- 部材タブの部材一覧はパネルを含む全要素を並べ、種別を `{:?}`（`PanelZone`）で表示する。
  ...
  折りたたみは今後の検討事項。
- ナビゲータの 2 グループはパネルを数えないため、合計が部材一覧の行数と一致しなくなった。
  誤分類より不一致のほうが正しい状態だが、気になるなら「その他」グループを設ける余地がある。
  現状は利用者ドキュメント（`docs/preparation/06_仕口パネル.md`）に明記するにとどめた。
- 診断タブの断面未割当は `section` のみを見ており、`material` の未割当は警告しない。
  解析前チェック（`precheck_model`）は両方をエラーにするため、診断が E0/W0 でも解析が
  止まる組合せが残る。今回の報告（誤検知）とは逆方向の粒度差のため、本対応では広げていない。
- 選択ハイライトは選択部材ごとに `elements.iter().find()` する O(部材数 × 選択数)。
  グループ選択で大規模モデルを丸ごと選ぶと効くが、本対応の前から同じ実装であり、
  今回はナビゲータ側で選択数が減る方向のため触っていない。
