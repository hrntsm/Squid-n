...
## 訂正（2026-09-16）: 壁の支持先は割当領域の境界を正本とする

上記の是正で入れた「支持区間重複での診断」は、同じブランチの後続作業で支持先を壁版割当領域の
境界から引く方式へ変えたため撤去した。現在は各辺の支持部材と材軸区間を割当領域の境界から
そのまま使い、節点座標からの支持先再推定をしない（`wall_plate_load` の `SupportIndex` は削除）。
境界の支持部材が実在し、種別が適合し、材軸を解決できることは `Model::validate` が確認する。
