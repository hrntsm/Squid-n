...
## 6. 追補: ログ行の折り返し表示（2026-07-27）

改修当初はログ 1 件を 1 行に固定していた（メッセージ中の改行を空白へ畳み、
`Label::truncate()` で右端を「…」に切り、全文はホバーで補う方式。ステータスバーの
`last_error` 表示に合わせたもの）。しかし ST-Bridge 取り込み時の注意のように
複数項目を列挙する長文では冒頭しか読めず、ホバーしないと内容が分からなかった。

`crates/squid-n-app/src/app/mod.rs` の下ドック `BottomTab::Log` の描画を
`Label::truncate()` から `Label::wrap()` へ変更し、あわせて `replace('\n', " ")` の
畳み込みも廃止した。これによりパネル幅での自動折り返しと、メッセージ自身が持つ
改行の両方が効き、全文がパネル内に表示される。全文が見えるためホバー
（`on_hover_text`）は不要になり削除した。

- 折り返し幅は外側の `ScrollArea::vertical()`（`auto_shrink([false, false])`）が
  与えるパネル幅で決まるため、ドック幅の変更に追従する。
- ステータスバーの `last_error`/`last_notice` は 1 行表示のままで変更していない
  （高さが可変になるとレイアウトが揺れるため、そちらは truncate＋ホバーを維持）。
