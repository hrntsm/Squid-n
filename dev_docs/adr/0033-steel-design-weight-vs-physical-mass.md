...
- **設計重量（DL 荷重・地震用重量）**: 設計用単位体積重量 γs = 78.5 kN/m³。
  基準資料の単位体積重量表（鉄骨）による。物理質量密度 ρ に比例させ、標準密度
  7.85 t/m³ のとき 78.5 kN/m³ とする（γs = 78.5 × ρ/7.85 kN/m³。質量密度 0 の材料は
  自重 0）。これは物理質量密度に g ≒ 10 m/s² を掛けた設計重量に相当する。
- **物理質量（質量行列・動的解析）**: 物理質量密度 ρ = 7.85 t/m³。
  質量密度 `Material.density` はこの物理質量密度のまま（鋼材 7.85e-9 t/mm³）とし、
  設計重量はこの密度に比例させて導出する（設計重量から物理質量は導出しない）。
...
設計重量は物理質量密度に比例させる。標準密度では基準資料の 78.5 kN/m³ を保ちつつ、
質量密度 0 の材料に自重を生じさせないためである（γs = 78.5 を密度に依らず固定すると、
質量 0 の材料にも自重が発生し、荷重ケースの自動生成を通じて解析が成立してしまう）。
...
- 鋼材を含む DL 荷重・地震用重量が増える（78.5/77.0 ≈ +1.9%。従来の質量換算比
  7.85 t/m³ との比では 78.5/76.98 ≈ +2.0%）。ただし標準密度以外の鋼材では設計重量が
  密度に比例して変わる。
