...
**実装参照**：`squid_n_io::wave_library::{wave_library_dir, list_wave_library, register_wave, wave_sha256}`（`crates/squid-n-io/src/wave_library.rs`）が保管先の解決・一覧・登録・ハッシュ算定を担います。
...
**実装参照**：保存先は `squid_n_io::wave_library::wave_library_dir`、一覧は同 `list_wave_library`（`crates/squid-n-io/src/wave_library.rs`）が持ち、一覧とファイル選択ダイアログは `.csv`・`.txt`・`.dat` の拡張子でフィルタします。
...
**実装参照**：`squid_n_app::app::App::{register_wave_to_library_dialog, confirm_register_wave}`（`crates/squid-n-app/src/app/actions/wave_library.rs`）が受け付け、コピーは `squid_n_io::wave_library::register_wave` が行います。
...
**実装参照**：ドロップダウンは `squid_n_app` の解析パネル（`crates/squid-n-app/src/app/panels/analysis.rs`）が表示し、実行は `squid_n_app::app::App::run_time_history_from_library` と `App::start_lumped_mass_library_th_job`（`crates/squid-n-app/src/app/actions/`）が担います。
...
**実装参照**：保存は `squid_n_app::app::App::save_project_to_opts` が波形名と SHA-256 を `SavedAnalysisSettings` に同梱し、復元は `App::{restore_wave_library_selection, restore_lumped_wave_library_selection}`（`crates/squid-n-app/src/app/actions/wave_library.rs`）が行います。
