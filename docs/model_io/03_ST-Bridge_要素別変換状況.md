...
| `StbWall`（壁） | ✅ | ✅ | 境界節点ループ＋断面参照（厚さ）。`id_material` は断面へ移す。開口（`StbOpen`）は対象外。取り込み先は壁版（囲まれた `WallPlate`）で、解析要素は都度生成する。解析要素になるかは [4.5 壁エレメントモデル](../calc_basis/04_要素剛性/05_壁エレメントモデル.md) の条件で決まり、ならない壁版は荷重だけを持つ壁版として扱う。どの壁領域にも載らない壁版のうち、水平な辺がちょうど 1 つ同一レベルの大梁に全長載り自由端が 1〜2 点のものは取り付く壁版へ自動変換する（張り出しの正は鉛直上向き。層内の腰壁・垂れ壁は壁領域へ帰属した囲まれた壁版のまま）。書き出しは壁版から出すため、4 節点でない囲まれた壁版も往復するが、取り付く壁版は往復しない。解析要素にならない壁版も、自重は自動で分配し地震用重量・「DL」・数量拾いへ算入する（[1.5 地震用重量・層データの生成](../calc_basis/01_荷重/05_地震用重量_層データの生成.md)） |
