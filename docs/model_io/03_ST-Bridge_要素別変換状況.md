...
ST-Bridge の ID は `positiveInteger`（1 始まり）に合わせて出力し、取り込み時に内部の 0 始まり連番へ正規化します。
...
| 断面のグレード名（`strength_main`・`strength_concrete`・`strength_steel` ほか） | ✅ | ✅ | 材料の実体はこれで往復する。取り込みでは名前から[標準材料一覧](../calc_basis/02_材料/04_標準材料一覧.md)で E・ν・密度・Fc・Fy を復元する。区分を名前から決められない材料は物性から推定し、取り込み報告で通知する |
...
| `StbBeam`（小梁） | ✅ | ✅ | 二次部材として往復する。全体解析の対象外で、床荷重と自重は大梁への集中荷重（CMQ）として伝える。取り込み後は「床」タブの小梁（二次部材）一覧に表示され、端部支持条件を設定できる（実部材化済みは除く）。幾何的に支持のない端（節点が実部材に接続せず、大梁の材軸上にも他の二次部材の内法にもない端。許容差 10 mm）は自由端（片持ち小梁）として取り込み、件数と対象を取り込み報告に出す。他の二次部材の端点と一致する端は、自分の反対側の端が主架構に載り、かつ相手の材軸が連続していないときだけ自由端にする。断面検定は [小梁の検定](../calc_basis/06_一次設計/05_小梁・床の断面検定.md#小梁の検定)（二次部材経路。床板分配の線荷重を単純梁または片持ち梁として重ね合わせる）。両端のいずれかがモデル節点に一致しない小梁は、ST-Bridge が材端を節点 ID で表すため書き出せず、書き出し全体がエラーになる（下記参照） |
| `StbPost`（間柱） | ✅ | ✅ | 二次部材の間柱として往復。節点は `id_node_bottom`/`_top`（`id_node_start`/`_end` も可）。両端のいずれかがモデル節点に一致しない間柱は、小梁と同じく書き出せず、書き出し全体がエラーになる（下記参照） |
| `StbBrace`（ブレース） | ✅ | ✅ | `feature_brace` を読み、`TENSIONANDCOMPRESSION` 以外は引張専用とする。両端ピンで取り込む |
| `StbSlab`（スラブ） | ✅ | ⚠️ | 境界節点ループ（`StbNodeIdOrder` のテキスト・CDATA・子要素 `StbNodeId` のいずれも可）＋断面参照。取り込み後、各大梁・小梁から床板割当領域を再構築し、スラブと割当領域の交差面積が正の領域ごとに床板（Slab）を 1 枚作る。仕上げ荷重・積載は元スラブの値へ交差面積比を掛けて領域へ合算し、躯体の断面・材料は元スラブから継承する（1 領域 1 版）。1 辺が大梁に全長載り自由端が 1〜2 点なら取り付く床板へ変換する。どの割当領域にも入らない面積が正なら、その面積と境界節点を含むエラーにし、取り込み全体を失敗させる。`kind_slab` は読まない。書き出しは囲まれかつ版がある床板だけ（`kind_slab` は `NORMAL` 固定）。仕上げ荷重・用途（積載）・分配法・取り付き・版なしは対象外 |
| `StbWall`（壁） | ✅ | ✅ | 境界節点ループ＋断面参照（厚さ）。`id_material` は断面へ移す。開口（`StbOpen`）は対象外。取り込み先は壁版割当領域で、柱・梁・間柱から再構築した割当領域のうち境界が一致するものへ壁版を割り当てる。解析要素は都度生成する。解析要素になるかは [壁エレメントモデル](../calc_basis/04_要素剛性/05_壁エレメントモデル.md) の条件で決まり、ならない壁版は解析要素にはしない。どの割当領域とも境界が一致しない壁は、取り付く壁版へ自動変換できるときだけ取り込み、できない場合は境界節点を含むエラーにして取り込み全体を失敗させる（境界が間柱で分割された領域への面積按分は未実装のため、黙って落とさない）。書き出しは壁版から出すため、4 節点でない囲まれた壁版も往復するが、取り付く壁版は往復しない。解析要素にならない壁版も、自重は自動で分配し地震用重量・「DL」・数量拾いへ算入する（[地震用重量・層データの生成](../calc_basis/01_荷重/05_地震用重量_層データの生成.md)）。仕上げ・増打ちの面荷重は対応する要素がないため往復せず、取り込んだ壁版では常に空になる |
...
囲まれた壁版の自重支持辺の負担率と間柱の端部負担率は、ST-Bridge では取り込み・書き出しを行いません。
ST-Bridge 取込後は、モデルタブの「壁版」一覧で、解析要素にならない壁版の自重支持辺の負担率を画面から指定してください
（[壁の断面と自重](../calc_basis/01_荷重/09_壁の断面と自重.md)）。値の保持には `.scz` を使用します。
...
床領域の付け直しで、どの床領域にも載らない版・所属の付かない小梁・面積照合できなかった旧床領域があるときも、件数を警告へ出します。
...
材軸中間へアンカーした二次部材（両端のいずれかが一致するモデル節点を持たない小梁・間柱）は、ST-Bridge が
...
| `StbSecWall_RC`（厚さ） | ⚠️ | ✅ | 取り込むのは厚さのみで、符号・階・`strength_concrete` は読まない。書き出しは壁版ごとに 1 件、符号は `W1` のような自動命名 |
