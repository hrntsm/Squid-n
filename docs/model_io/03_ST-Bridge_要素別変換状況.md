...
| `StbWall`（壁） | ✅ | ✅ | 境界節点ループ＋断面参照（厚さ）。`id_material` は断面へ移す。開口（`StbOpen`）は対象外。取り込み先は壁版（囲まれた `WallPlate`）で、解析要素は都度生成する。境界が 4 節点でない壁版と断面未割当の壁版は解析要素にせず警告する。どの壁領域にも載らない壁版のうち、水平な辺がちょうど 1 つ同一レベルの大梁に全長載り自由端が 1〜2 点のものは取り付く壁版へ自動変換する（張り出しの正は鉛直上向き。層内の腰壁・垂れ壁は壁領域へ帰属した囲まれた壁版のまま）。書き出しは壁版から出すため、4 節点でない囲まれた壁版も往復するが、取り付く壁版は往復しない。取り付く壁版は解析要素にせず、自重の自動分配も行わない |
