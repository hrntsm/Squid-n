...
| 床・壁 | スラブ（境界節点ループ＋断面。符号・階・板厚・コンクリート材料。RC・デッキ合成 `StbSecSlabDeck`）、壁（境界節点ループ＋厚さ＋材料）。取り込み後、各大梁・小梁から床板割当領域を再構築し、スラブと割当領域の交差多角形の面積を求めます。交差面積が正の割当領域ごとに床板（Slab）を 1 枚作り、仕上げ荷重・積載は元スラブの値へ交差面積比（元スラブ面積に対する割合）を掛けて領域へ合算し、躯体の断面・材料は元スラブから継承します（1 領域 1 版。複数領域をまたぎません）。水平大梁にちょうど 1 辺が全長載り自由端が 1〜2 点の版は取り付く床板へ変換します（張り出しの正は取付き線の左側、区間は全長、荷重の出口は取付き線への分布）。どの割当領域にも入らない面積が正なら、その面積と元の境界節点を含むエラーにして取り込み全体を失敗させます（面積を欠落させない）。`kind_slab` は読みません。書き出しの `StbSlab` は囲まれかつ版がある床板だけ、`kind_slab` は `NORMAL` 固定です（版なし・取り付きは出しません）。壁は柱・梁・間柱から再構築した壁版割当領域のうち境界が一致するものへ壁版として割り当てます。一致する割当領域が無い壁は、取り付く壁版（パラペット、および構面の外へ張り出す腰壁・垂れ壁に相当）へ自動変換できるときだけ取り込み（条件と既定値は [壁の断面と自重](../calc_basis/01_荷重/09_壁の断面と自重.md)）、それもできない場合は境界節点を含むエラーにして取り込み全体を失敗させます。解析要素にならない壁版も、自重は自動で分配し地震用重量・「DL」・数量拾いへ算入します（[地震用重量・層データの生成](../calc_basis/01_荷重/05_地震用重量_層データの生成.md)）。解析用の壁要素はモデルに残しません。書き出しは壁版から `StbWall` を出します（4 節点でない壁版も往復します。解析要素になるのは [壁エレメントモデル](../calc_basis/04_要素剛性/05_壁エレメントモデル.md) の条件を満たす囲まれた壁版だけです。取り付く壁版は往復対象外です） |
...
- 基礎・杭・フーチング、開口、`StbParapet` 要素（`StbWall` として書いたパラペットは壁版として取り込み、条件を満たせば取り付く壁版へ変換します。[壁エレメントモデル](../calc_basis/04_要素剛性/05_壁エレメントモデル.md)）。
- 剛域・製作情報などの詳細属性。
