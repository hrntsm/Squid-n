...
| 部材 | 柱（鉛直材）／大梁（水平材）／間柱／ブレース（斜材・`feature_brace`）。向き `rotate`、端部接合 `condition_*`、節点・断面の参照 |
| 床・壁 | スラブ（境界節点ループ＋断面。符号・階・板厚・コンクリート材料。RC・デッキ合成 `StbSecSlabDeck`）、壁（境界節点ループ＋厚さ＋材料） |
