...
このセクションは、Squid-N が行う各計算について、どの基準や法令のどの式で算定しているかを体系的に示す。
...
まず導入文で、何を算定するか、そしてその根拠となる法令（建築基準法施行令）の条番号、告示番号、学会規準名、文献のページを述べる。
続いて **算定式** として、Squid-N が実装している式そのものを、定数、分岐条件、クランプ範囲まで含めて示す。
...
| # | ドキュメント | 主な内容 | 主な根拠 |
|---|-------------|----------|----------|
| 1 | [荷重・外力](./01_荷重/README.md) | 固定・積載・積雪・風・地震力（Z・Rt・Ai・C0・T）、荷重組合せ | 令82〜88条、告1793、告1454 |
| 2 | [材料構成則・材料強度](./02_材料/README.md) | 一軸履歴則、コンクリート／鋼の応力ひずみ、F値、許容応力度の材料基礎 | 令90〜91条、令96・98条、告示 |
| 3 | [断面性能](./03_断面性能/README.md) | 断面諸量（A・I・Z・Zp）、せん断形状係数、ヤング係数、SRC/CFT 等価剛性、ファイバー断面 | RC規準、各規準 |
| 4 | [要素の定式化・剛性](./04_要素剛性/README.md) | ティモシェンコ梁、剛域、CMQ、MITC4シェル、壁エレメント、パネルゾーン、ばね | 力学（マトリクス法・FEM） |
| 5 | [構造解析](./05_構造解析/README.md) | 線形静解析、固有値、幾何剛性、時刻歴（Newmark-β/HHT）、減衰、非線形解法、プッシュオーバー | 力学（数値解析法） |
| 6 | [一次設計（許容応力度計算）](./06_一次設計/README.md) | RC・S・SRC の断面検定、接合部、付着・定着、座屈長さ | 令82条、RC規準、鋼構造規準 |
| 7 | [二次設計（保有水平耐力計算）](./07_二次設計/README.md) | 保有水平耐力 Qu、必要保有 Qun、Ds、部材ランク、幅厚比、剛性率 Fs、偏心率 Fe、層間変形角 | 令82条の2〜6、告1792 |
| 8 | [部材終局耐力](./08_部材終局耐力/README.md) | RC/S/SRC/CFT の Mu・Qmu・Qsu（荒川式・塑性理論式）、耐震壁非線形、接合部終局 | 靭性保証型指針、終局強度型指針、技術基準解説書 |
| 9 | [限界耐力計算](./09_限界耐力計算/README.md) | 加速度応答スペクトル So・Gs、容量スペクトル法（性能点）、Fh | 告1457、令82条の5 |
...
内部計算は原則として SI 単位系（N, mm, N/mm²=MPa）で行う。
荒川式のように工学単位系（kgf/cm²）で導出された実験式は、単位換算（1 kgf/cm² = 0.0980665 N/mm²、1 kgf = 9.80665 N）を施してから評価する。
該当する箇所は各ページで明記する。
...
このドキュメントは実装が依拠する根拠を説明するものであり、個々の建築物の設計における式の適用可否（適用範囲や前提条件）の判断は、設計者（構造設計一級建築士等）の責任に属する。
