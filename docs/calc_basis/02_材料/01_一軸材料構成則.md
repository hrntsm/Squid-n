...
このモデルは、弾塑性力学の線形移動硬化（kinematic hardening）に基づきます。
コード上に規準名を明示するコメントはありません。
...
\\( f \le 0 \\) のとき弾性（接線 E）とします。
\\( f > 0 \\) のとき戻し写像により塑性ひずみと応力を更新します。
...
**実装**: `squid_n_material::uniaxial::Bilinear`（`uniaxial.rs:64`）が算定します。
既定硬化率は用途側で指定し、ファイバー要素では 0.01 とします。

## 2.1.2 Menegotto–Pinto 鉄筋モデル

このモデルは、バウシンガー効果を平滑に表現する Menegotto–Pinto モデルに基づきます。
ξ（塑性ひずみ蓄積）の更新は Chang & Mander (2004) 系の簡易形であり、コードにコメントとして記載しています。
...
**実装**: `uniaxial::MenegottoPinto`（`uniaxial.rs:185`）が算定します。
既定パラメータは \\( b = 0.01,\ R_0 = 20.0,\ a_1 = 18.5,\ a_2 = 0.15 \\) であり、反転時に漸近線を鏡映更新します。
