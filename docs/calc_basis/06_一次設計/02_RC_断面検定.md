...
**実装**：`rc::column`（`rc/column.rs::column_check`）が検定します。地震時 QD は軸力を考慮する `rc_column_mu_simple` が担います。
...
RC 梁の付着・定着を、RC規準1999（既定）／1991 に基づいて検定します。
...
\\( \gamma_1 = 1 - l_0/l \\)、\\( \gamma_2 = 1 - r_0 \\)、\\( \gamma_3 = 1 - h_0/h \\)。終局せん断（[8.4.2](../08_部材終局耐力/04_RC_部材_非線形復元力特性.md)）の開口低減と同一の式です。剛性計算用の \\( r_1 = 1 - 1.25 r_0 \\)（[4.5 壁エレメントモデル](../04_要素剛性/05_壁エレメントモデル.md)）とは別式。
