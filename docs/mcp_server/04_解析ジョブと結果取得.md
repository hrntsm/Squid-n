...
**実装**：ジョブの登録・実行・状態は `squid_n_mcp::server::SquidNServer::{analysis_run, analysis_status}`（`crates/squid-n-mcp/src/server.rs`）が、結果の取得は `squid_n_mcp::persist::result_get_json`（`crates/squid-n-mcp/src/persist.rs`）が担います。
...
**実装**：スキーマは `squid_n_io::results::{nodal_disp_schema, member_force_schema, modal_schema, time_history_schema}`（`crates/squid-n-io/src/results/schema.rs`）が定め、結果の書き込みは `squid_n_mcp::persist::persist_job_outcome`（`crates/squid-n-mcp/src/persist.rs`）が行います。
